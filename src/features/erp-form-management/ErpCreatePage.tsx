...
export function ErpCreatePage({ config }: { config: ErpFormConfig }): ReactElement {
  ...
  const [lines, setLines] = useState<Record<string, FormValue>[]>(() => (config.lineFields ? [defaults(config.lineFields)] : []));
  ...
  const submit = async (e: FormEvent) => {
    ...
    const scopedHeader = config.fields.some((field) => field.key === 'branchId') ? { ...header, branchId: activeBranchId } : header;
    if (config.fields.some((field) => field.key === 'branchId') && activeBranchId <= 0) {
      toast.error(
        t('common.activeBranchRequired', {
          defaultValue: 'Aktif şube bilgisi bulunamadı. Lütfen yeniden giriş yapın.',
        }),
      );
      return;
    }
    ...
    if (missingHeader || missingLine) {
      toast.error(
        t('common.requiredFields', {
          defaultValue: 'Zorunlu alanları doldurun.',
        }),
      );
      ...
      toast.success(
        editId !== null
          ? t('common.updateSuccess', {
              defaultValue: 'Kayıt başarıyla güncellendi.',
            })
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5">
      <section className="metivon-hero rounded-3xl p-6">
        <button type="button" onClick={() => navigate(config.returnPath)} className="mb-4 inline-flex items-center gap-2 text-sm text-white/80">
        ...
        <p className="mt-2 text-white/75">
          {t(`forms.${config.returnPath}.description`, {
      ...
      <form onSubmit={submit} noValidate className="space-y-5">
          ...
          <section className="metivon-panel rounded-2xl border p-5">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('common.documentLinesHint')}</p>
              </div>
              <Button type="button" variant="outline" onClick={() => setLines((v) => [...v, defaults(config.lineFields!)])}>
                <Plus />
                {t('common.addLine')}
              </Button>
            </div>
              ...
              {lines.map((line, index) => (
                <div key={index} className="rounded-xl border bg-muted/20 p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="text-sm font-semibold">{t('common.line', { number: index + 1 })}</span>
                    <Button type="button" size="icon-sm" variant="ghost" disabled={lines.length <= (config.minimumLines ?? 1)} onClick={() => setLines((v) => v.filter((_, i) => i !== index))}>
                      <Trash2 />
                    </Button>
                  </div>
                            ...
                            v.map((x, i) =>
                              i === index
                                ? {
                                    ...x,
                                    [f.key]: value,
                                    ...Object.fromEntries(config.lineFields!.filter((candidate) => candidate.filterBy === f.key).map((candidate) => [candidate.key, ''])),
                                  }
                                : x,
                            ),
                          )
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate(config.returnPath)}>
            {t('common.cancel')}
          </Button>
          <Button type="submit" disabled={saving || lookupQuery.isLoading || detailQuery.isLoading}>
            {saving
              ...
              : editId !== null
                ? t('common.edit')
                : t(`forms.${config.returnPath}.submit`, {
...
function Field({ field, values, lookups, onChange, onPatch, invalid }: { field: FormField; values: Record<string, FormValue>; lookups: ErpLookups; onChange: (value: FormValue) => void; onPatch: (patch: Record<string, FormValue>) => void; invalid: boolean }) {
  ...
  const anchorPurchaseOrder = field.key === 'purchaseOrderIds' ? (lookups.purchaseOrders ?? []).find((order) => selectedPurchaseOrderIds.includes(order.id)) : undefined;
  const options = field.key === 'purchaseOrderIds' && anchorPurchaseOrder ? baseOptions.filter((order) => String(order.supplierId) === String(anchorPurchaseOrder.supplierId) && String(order.warehouseId) === String(anchorPurchaseOrder.warehouseId) && String(order.tradeDossierId ?? '') === String(anchorPurchaseOrder.tradeDossierId ?? '')) : baseOptions;
  ...
  const selectedBranchId = Number(values[field.branchField ?? 'branchId']) || Number(lookups.warehouses?.find((x) => x.id === selectedWarehouseId)?.branchId) || Number(activeBranch?.id) || undefined;
  ...
  const serialOptions = (lookups[field.lookup ?? 'inventorySerials'] ?? []).filter((x) => (!serialProductId || Number(x.productId) === serialProductId) && (!selectedWarehouseId || !x.warehouseId || Number(x.warehouseId) === selectedWarehouseId));
  return (
    <div className={`space-y-2 [&_[aria-invalid=true]]:border-destructive [&_[aria-invalid=true]]:ring-2 [&_[aria-invalid=true]]:ring-destructive/20 ${field.span === 2 ? 'md:col-span-2' : ''}`}>
      <Label>
        ...
        {field.required ? (
          <span className="ms-1 font-bold text-destructive" aria-hidden="true">
            *
          </span>
        ) : null}
      ...
      {field.key === 'branchId' ? (
        <Input value={activeBranch?.name ?? ''} disabled readOnly aria-invalid={invalid} data-active-branch-id={activeBranch?.id ?? ''} />
      ) : field.type === 'number-series' ? (
        <NumberSeriesCombobox
          module={field.numberSeriesModule!}
          reference={field.numberSeriesReference!}
          branchId={selectedBranchId}
          warehouseId={selectedWarehouseId}
          value={String(value)}
          onChange={onChange}
          placeholder={t('numberSeries.select', {
            defaultValue: 'Numara serisi seçin',
          })}
          invalid={invalid}
        />
        ...
        <SerialEntryDialog
          value={String(value)}
          onChange={onChange}
          expectedQuantity={field.quantityField ? Number(values[field.quantityField] ?? 0) : undefined}
          inventoryOptions={field.type === 'serial-select' ? serialOptions : undefined}
          onGs1Data={
            field.type === 'serial-entry'
              ? (data) =>
                  onPatch({
        ...
        <Textarea required={field.required} aria-invalid={invalid} value={String(value)} onChange={(e) => onChange(e.target.value)} />
      ) : field.type === 'checkbox' ? (
        ...
        <Input required={field.required} aria-invalid={invalid} type={field.type} step={field.step} value={String(value)} onChange={(e) => onChange(field.type === 'number' ? (e.target.value === '' ? '' : Number(e.target.value)) : e.target.value)} />
      )}
    </div>
  );
}
