...
import { getPermissionGroupDisplayDescription, getPermissionGroupDisplayName } from '../utils/permission-group-presets';
                ...
                <label htmlFor={`group-${item.id}`} className="flex min-w-0 flex-1 cursor-pointer items-center gap-2">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className={cn('block text-sm font-semibold', isChecked ? 'text-primary dark:text-primary' : 'text-slate-800 dark:text-slate-100')}>
                      {getPermissionGroupDisplayName(t, item)}
                    </span>
                    {getPermissionGroupDisplayDescription(t, item) && (
                      <span className="mt-0.5 block truncate text-xs text-slate-500 dark:text-slate-400">{getPermissionGroupDisplayDescription(t, item)}</span>
                    )}
                  </span>
                  {item.isSystemAdmin && (
                    <Badge
