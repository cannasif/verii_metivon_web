...
import { Eye, KeyRound, Loader2, Plus, RefreshCw, RotateCcw, ShieldCheck, Users2, Edit2, Trash2 } from 'lucide-react';
...
export function PermissionGroupsPage(): ReactElement {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['id', 'name', 'isSystemAdmin', 'isActive', 'permissionCount']);
  const [columnOrder, setColumnOrder] = useState<string[]>(['id', 'name', 'isSystemAdmin', 'isActive', 'permissionCount']);
  ...
  useEffect(() => {
    const prefs = loadColumnPreferences(PAGE_KEY, user?.id, ['id', 'name', 'isSystemAdmin', 'isActive', 'permissionCount']);
  ...
  const handleEditClick = (item: PermissionGroupDto): void => {
    if (!canUpdate) return;
    if (item.isSystemAdmin || item.isSystem) return;
    setEditingItem(item);
    setFormOpen(true);
  };
  ...
  const handleRestoreDefaults = async (): Promise<void> => {
    ...
    try {
      ...
      await ensurePermissionCatalogReady();
      const restored = await permissionGroupApi.restoreDefaults();
      await queryClient.invalidateQueries({ queryKey: ['permissions'] });
      toast.success(t('permissionGroups.restoreDefaults.success', {
        count: restored.length,
        defaultValue: `${restored.length} hazır yetki grubu eski haline getirildi.`,
      }));
  ...
  const handleFormSubmit = async (formData: CreatePermissionGroupSchema): Promise<void> => {
    if (editingItem?.isSystemAdmin || editingItem?.isSystem) return;
    if (editingItem) {
      const updateDto = {
        name: formData.name,
  ...
  const baseColumns = [
    { key: 'id', label: t('permissionGroups.table.id', { defaultValue: 'Kayıt ID' }) },
  ...
  const filterColumns = useMemo<FilterColumnConfig[]>(() => [
    { value: 'id', type: 'number', labelKey: 'permissionGroups.table.id' },
  ...
  const exportRows = useMemo<Record<string, unknown>[]>(
    () =>
      filteredItems.map((item) => ({
        id: item.id,
        name: getPermissionGroupDisplayName(t, item),
  ...
  const columns: DataTableGridColumn<PermissionGroupColumnKey>[] = useMemo(
    () => [
      { key: 'id', label: t('permissionGroups.table.id', { defaultValue: 'Kayıt ID' }), sortable: true, cellClassName: 'font-mono font-semibold' },
      ...
      {canUpdate && (
        <>
          <Button
            ...
            title={t('permissionGroups.previewPermissions')}
          >
          ...
          <Button
            ...
            title={item.isSystem ? t('permissionGroups.builtInLocked', 'Hazır sistem grubu değiştirilemez') : undefined}
          >
  ...
  return (
    <div className="w-full space-y-6">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between relative z-10">
          ...
          {(canCreate || canUpdate) && (
            <div className="flex shrink-0 flex-wrap gap-2">
              {canUpdate && (
                <Button type="button" variant="outline" onClick={() => setRestoreDialogOpen(true)} className="h-12 shrink-0 px-5">
                  <RotateCcw size={18} className="mr-2" />
                  {t('permissionGroups.restoreDefaults.button', { defaultValue: 'Hazır Yetki Gruplarını Oluştur' })}
                </Button>
              )}
              {canCreate && (
                <Button
          ...
          <div className={MANAGEMENT_LIST_TABLE_SHELL_CLASSNAME}>
            <ManagementDataTableChrome>
              <DataTableGrid<PermissionGroupDto, PermissionGroupColumnKey>
                ...
                rowKey={(r) => r.id}
                renderCell={(row, key) => {
                  if (key === 'id') return <span className="font-mono font-semibold">{row.id}</span>;
                  if (key === 'name') return <span className="font-medium">{getPermissionGroupDisplayName(t, row)}</span>;
      ...
      <GroupPermissionsPanel groupId={permissionsPanelGroupId} open={permissionsPanelOpen} onOpenChange={setPermissionsPanelOpen} readOnly={!canUpdate} />

      <Dialog open={restoreDialogOpen} onOpenChange={setRestoreDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t('permissionGroups.restoreDefaults.title', { defaultValue: 'Hazır yetki gruplarını yeniden oluştur' })}</DialogTitle>
            <DialogDescription>
              {t('permissionGroups.restoreDefaults.description', { defaultValue: 'Sistem yöneticisi, mal kabul, depo, sevkiyat ve diğer hazır görev grupları güvenli varsayılan izinleriyle yeniden oluşturulur. Eksik veya değiştirilmiş bağlantılar düzeltilir; özel gruplarınız etkilenmez.' })}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setRestoreDialogOpen(false)} disabled={isRestoringDefaults}>{t('common.cancel')}</Button>
            <Button onClick={handleRestoreDefaults} disabled={isRestoringDefaults}>
              {isRestoringDefaults ? <Loader2 className="mr-2 size-4 animate-spin" /> : <RotateCcw className="mr-2 size-4" />}
              {t('permissionGroups.restoreDefaults.confirm', { defaultValue: 'Yeniden Oluştur' })}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
