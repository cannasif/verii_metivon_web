...
export const useMyPermissionsQuery = () => {
  ...
  const permissions = isValidPermissionSnapshot(cachedPermissions, userId) ? cachedPermissions : null;
  ...
  useEffect(() => {
    if (!userId || !cachedPermissions || permissions) return;
    usePermissionsStore.getState().clearPermissions(userId);
  }, [cachedPermissions, permissions, userId]);
  ...
  return useMemo(
    () => ({
      data:
        permissions ??
        (isValidPermissionSnapshot(cacheEntry?.data, userId) ? cacheEntry.data : null),
