...
function resolveLocalizedApiErrorMessage(payload: unknown, statusCode?: number): string | null {
  const language = (i18n.resolvedLanguage || i18n.language || 'tr').split('-')[0].toLowerCase();
  ...
  return String(
    i18n.t(`common:apiErrors.${key}`, {
      defaultValue: i18n.t('common:apiErrors.generic'),
    }),
  );
}
...
api.interceptors.request.use((config) => {
  ...
  const tunnel = resolveIisSafeHttpMethod(config);
  const originalMethod = tunnel.originalMethod;
  config.method = tunnel.method;
  config.url = tunnel.url;

  if (originalMethod === 'get') {
    const pagedTransport = resolvePagedPostTransport(config);
    if (pagedTransport.tunneled) {
      config.method = pagedTransport.method;
      config.url = pagedTransport.url;
      config.params = pagedTransport.params;
      config.data = pagedTransport.data;
    } else {
  ...
  if (branchCode != null) {
    ...
    trackApiRequestEnd();
    return Promise.reject(new Error(String(i18n.t('common:apiErrors.validation'))));
  }
