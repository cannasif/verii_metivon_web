const ROUTE_NAMESPACE_ENTRIES: Array<{ match: (pathname: string) => boolean; namespaces: string[] }> = [
  ...
  { match: (pathname) => pathname.startsWith('/accounts'), namespaces: ['business-partner-management'] },
  ...
  {
    match: (pathname) => [
      '/warehouses', '/inventory', '/purchase-orders', '/goods-receipts',
      '/transfer-orders', '/sales-orders', '/pricing/price-lists', '/shipments',
      '/inventory-counts', '/e-documents', '/accounting', '/import-dossiers', '/trade-dossiers',
...
export const GLOBAL_ROUTE_NAMESPACES = [
  'common',
  'notification',
  'activity-image',
  'ai-assistant',
  // NumberFormatBuilder is a reusable shared component used by multiple ERP route groups.
  // It must be ready before any of those pages render, not inferred from a single URL segment.
