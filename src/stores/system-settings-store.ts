...
const SUPPORTED_NUMBER_FORMATS = new Set([
  'tr-TR', 'en-US', 'de-DE', 'fr-FR', 'es-ES', 'it-IT', 'pt-PT', 'nl-NL',
  'pl-PL', 'ru-RU', 'ar-SA', 'fa-IR', 'ja-JP', 'ko-KR', 'zh-CN',
]);
...
export function normalizeSystemSettings(
  ...
  return {
    ...
    amountDecimalPlaces: settings?.amountDecimalPlaces ?? settings?.decimalPlaces ?? 2,
    priceDecimalPlaces: settings?.priceDecimalPlaces ?? 4,
    quantityDecimalPlaces: settings?.quantityDecimalPlaces ?? 3,
    exchangeRateDecimalPlaces: settings?.exchangeRateDecimalPlaces ?? 6,
    percentageDecimalPlaces: settings?.percentageDecimalPlaces ?? 2,
    costDecimalPlaces: settings?.costDecimalPlaces ?? 6,
    roundingMethod: settings?.roundingMethod ?? 1,
    useThousandsSeparator: settings?.useThousandsSeparator !== false,
    trimTrailingZeros: Boolean(settings?.trimTrailingZeros),
    defaultCurrencyId: settings?.defaultCurrencyId ?? 0,
    defaultCurrencyCode: settings?.defaultCurrencyCode ?? 'TRY',
    defaultCurrencyName: settings?.defaultCurrencyName ?? 'Turkish Lira',
    defaultCurrencySymbol: settings?.defaultCurrencySymbol ?? '₺',
    defaultCurrencyDecimalPlaces: settings?.defaultCurrencyDecimalPlaces ?? 2,
    currencyDisplay: settings?.currencyDisplay ?? 1,
    currencySymbolOnRight: settings?.currencySymbolOnRight !== false,
    dateFormat: settings?.dateFormat ?? 'dd.MM.yyyy',
    timeFormat: settings?.timeFormat ?? 24,
    timeZoneId: settings?.timeZoneId ?? 'Europe/Istanbul',
    firstDayOfWeek: settings?.firstDayOfWeek ?? 1,
    storeDateTimeAsUtc: settings?.storeDateTimeAsUtc !== false,
    defaultPageSize: settings?.defaultPageSize ?? 20,
    maximumExportRows: settings?.maximumExportRows ?? 10000,
    allowUserDisplayOverrides: settings?.allowUserDisplayOverrides !== false,
...
interface SystemSettingsState {
  ...
  setSettings: (settings: Partial<SystemSettingsDto>, fetchedAt?: number) => void;
}
