...
import { NOTIFICATIONS_ENABLED } from '@/features/notification/notification-features';
    ...
    (set, get) => ({
      ...
      bootstrapAppShell: async ({ token, userId, force = false }) => {
        ...
        const runBootstrap = async (): Promise<void> => {
          ...
          const hasFreshPermissions =
            isValidPermissionSnapshot(permissionEntry?.data, userId) &&
            isFresh(permissionEntry?.lastFetchedAt, PERMISSIONS_CACHE_TTL_MS);
          ...
          const shouldFetchUnreadCount = NOTIFICATIONS_ENABLED &&
            (force || !isFresh(unreadEntry?.lastFetchedAt, APP_SHELL_UNREAD_COUNT_TTL_MS));
