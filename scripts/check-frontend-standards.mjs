...
function checkFile(file, content) {
  ...
  lines.forEach((line, index) => {
    ...
    if (
      file.endsWith(path.join('erp-form-management', 'configs.ts')) &&
      /key:\s*["'][A-Za-z][A-Za-z0-9]*Id["'][^}\n]*type:\s*["'](?:number|text)["']/.test(line)
    ) {
      addViolation(
        file,
        lineNumber,
        'foreign-key fields must use a searchable relation selector; users must never type database IDs manually.'
      );
    }
  });
}
